...
const BusinessImageSection = ({
  ...
  const renderConfirmation = useMemo(
    () => (
      <ConfirmationDialog
        ...
        modalTitle={t(
          "dashboard.business.settings.businessProfile.tabs.basicDetails.confirmation.title"
        )}
        description={t(
          "dashboard.business.settings.businessProfile.tabs.basicDetails.confirmation.description"
        )}
  ...
  return (
    <div className="img-gallery-block">
      <h3>
        {t(
          "dashboard.business.settings.businessProfile.tabs.basicDetails.form.image.title"
        )}
      </h3>
      {isUploading || isDetailLoading ? (
        <Loader />
      ) : (
          ...
          {images?.map(({ id, image }) => (
            ...
            >
              <Button
                ...
                onClick={() => onDeleteInitiate(id)}
              >
                x
              </Button>
              <img src={image} alt={`business-image-${id}`} />
            </div>
          ))}
          {renderConfirmation}
